...
Authors: David Mutchler, Vibha Alangar, Matt Boutell, Dave Fisher, Mark Hays,
         Aaron Wilkin, their colleagues, and Weizhou Liu.
...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        x = circle.center.x-circle.radius+(1/2)**k*circle.radius
        y = circle.center.y-circle.radius+(1/2)**k*circle.radius
        point = rg.Point(x, y)
        edge = (1/2) ** k * circle.radius*2
        square     = rg.Square(point, edge)
...
def run_test_draw_circles_from_rectangle():
    ...
    # Test 1:
    rectangle = rg.Rectangle(rg.Point(300,200), rg.Point(100,400))
    rectangle.fill_color = 'pink'
    rectangle.outline_color = 'yellow'
    draw_circles_from_rectangle(3,4, rectangle, window1)
    # Test 2:
    rectangle = rg.Rectangle(rg.Point(500, 250), rg.Point(340,400))
    ...
    # Test 3:
    rectangle = rg.Rectangle(rg.Point(300,300), rg.Point(210,180))
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(m):
        ...
        x = rectangle.corner_1.x+radius*(2*k+1)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        x1 = rectangle1.get_center().x-(1/2)*rectangle1.get_width()*k
        y1 = rectangle1.get_center().y+(1/2)*rectangle1.get_height()*k
        x2 = rectangle2.get_center().x-(1/2)*rectangle1.get_width()*k
        y2 = rectangle2.get_center().y+(1/2)*rectangle1.get_height()*k
